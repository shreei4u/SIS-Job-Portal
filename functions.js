...
let users = [
  {name:'Shreekant', email:'shreekant@shieldinfrasolutions.in', phone:'-', password:'Shree#2425@22267', role:'admin'}
];
let candidates = [
...
function doRegister(e){
  ...
  if(users.some(u=>u.email===email)){
    err.textContent='This email is already registered. Please login.'; err.style.display='block'; return false;
  }
  users.push({name,email,phone,password:pass,role});
  playThankYou();
  ...
  setTimeout(()=>{ const le=document.getElementById('loginEmail'); if(le) le.value=email; },50);
  return false;
}
...
function doLogin(e){
  ...
  if(!user){ err.style.display='block'; return false; }
...
const navByRole = {
  ...
  freelancer:[['Overview','overview'],['Profile','profile'],['Open Projects','projects'],['My Bids','bids']],
  trainer:[['Overview','overview'],['My Batches','batches'],['Certifications','certs']],
...
function renderTab(user, tab){
  ...
  heading.textContent = tab.startsWith('db-') ? (dbLabel(tab.replace('db-',''))+' Database') : (tab.charAt(0).toUpperCase()+tab.slice(1));
  sub.textContent = 'Signed in as '+user.email;

  if(tab==='overview'){
    c.innerHTML = overviewWidgets(user) + (user.role==='employer'||user.role==='admin' ? atsPanel() : '') + (user.role==='admin' ? crmSnapshotWidget() : '') + activityPanel(user);
...
function atsPanel(){
  let cols = stages.map((st,idx)=>{
    let cards = candidates.filter(c=>c.stage===idx).map((c,ci)=>{
      const realIdx = candidates.indexOf(c);
      const nextBtn = idx < stages.length-1 ? `<button class="btn3d small" onclick="moveCandidate(${realIdx})">Move → ${stages[idx+1]}</button>` : `<span style="color:var(--green);font-size:11px;font-weight:700;">✓ Completed</span>`;
      return `<div class="kcard"><b>${c.name}</b><span>${c.role}</span>${nextBtn}</div>`;
    }).join('');
    return `<div class="kcol"><h4>${st} <span>${candidates.filter(c=>c.stage===idx).length}</span></h4>${cards||'<span style="color:var(--text-dim);font-size:11px;">No candidates</span>'}</div>`;
  }).join('');
  return `<div class="panel"><h3>ATS · Applicant Tracking Pipeline</h3>
  <div style="margin-bottom:14px;"><button class="btn3d small red" onclick="addCandidate()">+ Add Candidate</button></div>
  <div class="kanban">${cols}</div></div>`;
}
function moveCandidate(i){
  candidates[i].stage = Math.min(candidates[i].stage+1, stages.length-1);
  const c = candidates[i];
  if(c.applicantEmail){
    const app = (applications[c.applicantEmail]||[]).find(a=>a.jobIdx===c.jobIdx);
    if(app) app.status = stages[c.stage];
  }
  logActivity('<b>'+c.name+'</b> moved to <b>'+stages[c.stage]+'</b>'+(c.role?' for '+c.role:''));
  playClick();
  renderTab(currentUser, 'ats');
}
